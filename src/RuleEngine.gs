...
const RuleEngine = {
  ...
  classifyThread(thread) {
    ...
    for (const rule of CONFIG.CLASSIFICATION_RULES) {
      ...
      if (criteria.from && !from.includes(criteria.from.toLowerCase()))
        match = false;
      if (match && criteria.domain && domain !== criteria.domain.toLowerCase())
        match = false;
      if (
        match &&
        criteria.subject &&
        !subject.includes(criteria.subject.toLowerCase())
      )
        match = false;
      if (
        match &&
        criteria.body &&
        !bodySnippet.includes(criteria.body.toLowerCase())
      )
        match = false;
      ...
      if (
        match &&
        criteria.category &&
        !threadLabels.includes(`category:${criteria.category.toLowerCase()}`)
      ) {
        match = false;
      }
      ...
      if (match) {
        Logger.debug(
          `Thread "${subject}" matched rule for label "${label}". Criteria: ${JSON.stringify(criteria)}`
        );
        labelsToApply.add(label);
        if (isPriority) {
          Logger.debug(
            'Priority rule matched. Stopping classification for this thread.'
          );
